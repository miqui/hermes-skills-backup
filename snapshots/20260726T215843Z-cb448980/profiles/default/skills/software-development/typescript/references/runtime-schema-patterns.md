> **Scope:** These are source-project conventions; apply a rule only when the target repository's configuration, dependencies, and local standards support it.
...
For workspace-specific schema and migration requirements, consult the target framework's own documentation and repository conventions.
...
For workspace-specific file structure and migration rules, follow the target framework's documentation and repository conventions.
