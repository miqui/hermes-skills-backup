...
## Verifying CLI/Command Claims (Don't Trust Prose)

Skills and submitted docs often assert a command exists ("Use `npx <pkg> <subcommand>`") that turns out to be stale, aspirational, or hallucinated. Prose review alone won't catch this — verify empirically before approving or citing the claim:

1. **Check registry metadata first.** For npm packages, fetch `https://registry.npmjs.org/<pkg>` and inspect `versions[<dist-tags.latest>].bin`. If `bin` is absent or doesn't contain the claimed subcommand, the CLI usage is very likely wrong (this caught a fabricated `npx hono request`/`npx hono docs` CLI in a Hono skill review — the published `hono` package has no `bin` field at all).
2. **Reproduce in a scratch sandbox** when the registry check is ambiguous: `mkdir /tmp/<pkg>_test && cd it && npm init -y && npm install <pkg> && npx <pkg> <claimed-args>`. A concrete failure (`could not determine executable to run`, `command not found`) is definitive proof, stronger than any registry inspection alone.
3. **Cross-check provenance for any secondary/companion tool** the doc leans on (e.g. a recommended CLI alongside the main package) via the package's `repository`/`maintainers` registry fields and the linked GitHub repo. Note if it's young (0.x, weeks old) or a personal/non-org project — legitimate but still raises Update Drift (AST07) risk and should be pinned to an exact version rather than left as bare `npx <pkg>`.
4. **Don't lean on general web search for currency checks** — search engines can rate-limit/CAPTCHA-block mid-review; the registry API + direct GitHub repo view + local reproduction are more reliable and faster than fighting a blocked search results page.

Treat every CLI/command claim in a reviewed skill as a testable assertion, not a fact — this is a required step under Phase 2 (Full artifact review) and Phase 4 (Update and dependency review) above, not an optional nicety.
