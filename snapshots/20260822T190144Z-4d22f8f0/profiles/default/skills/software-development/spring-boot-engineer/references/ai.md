...
Spring AI provides first-class Qdrant support via `spring-ai-starter-vector-store-qdrant`. Qdrant is a Rust-based vector search engine with strong filtering, payload storage, and horizontal scaling.
  ...
  ai:
    vectorstore:
      qdrant:
        ...
        # host: ${QDRANT_CLUSTER_URL}
        # port: 443
        # use-tls: true
        # api-key: ${QDRANT_API_KEY}
```

### QdrantClient with custom collection
...
public class QdrantConfig {
    ...
    public QdrantClient qdrantClient() {
        QdrantApi qdrantApi = new QdrantApi("http://localhost:6333");
        return new QdrantClient(qdrantApi);
    }
...
### Payload filtering (Qdrant-native)

Qdrant supports rich payload filtering with Spring AI's `FilterExpressionBuilder`:
...
public class QdrantSearchService {
    ...
    public List<Document> searchByCategoryAndTenant(String query, String category, String tenantId) {
            ...
            SearchRequest.query(query)
                ...
                .withFilterExpression(
                    builder.and(
                        ...
                        builder.eq("tenantId", tenantId)
                    ).build()
                )
        );
    }

    public List<Document> searchByNumericRange(String query, BigDecimal minPrice, BigDecimal maxPrice) {
        FilterExpressionBuilder builder = new FilterExpressionBuilder();
        ...
        return vectorStore.similaritySearch(
            SearchRequest.query(query)
                ...
                .withFilterExpression(
                    builder.and(
                        builder.gte("price", minPrice.doubleValue()),
                        builder.lte("price", maxPrice.doubleValue())
                    ).build()
                )
        );
    }
...
public class QdrantIngestionService {
    ...
    public void ingestWithPayload(List<DocumentChunk> chunks) {
...
### Qdrant vs PgVector vs Chroma — when to choose

| Store | Best for | Strengths |
|---|---|---|
| Qdrant | High-throughput search, rich filtering | Rust performance, payload filtering, horizontal scaling |
| PgVector | Postgres-native, transactional | ACID with your relational data, no separate service |
| Chroma | Quick prototyping, local dev | Simple setup, embedded mode |

Choose Qdrant when you need high query throughput, complex payload filtering, or horizontal scaling beyond a single Postgres instance. Choose PgVector when your vectors should live alongside relational data in the same ACID transaction.
