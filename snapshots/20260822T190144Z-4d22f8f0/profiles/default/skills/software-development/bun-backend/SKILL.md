...
3. **Verify Node compatibility instead of assuming it.** Bun 1.4 substantially improves Node.js compatibility, but native modules, runtime globals, test behavior, and bundling edges still need real checks in the target project.
...
- any `[install].linker` choice in `bunfig.toml`; `isolated` is an intentional opt-in with different `node_modules` layout semantics
- `tsconfig.json`
...
bun update
bun outdated
bun pm diff <package>@<candidate-version>
```
...
- Before accepting a dependency upgrade, use `bun pm diff` where applicable and review newly introduced install scripts or sensitive runtime imports; package installation remains a supply-chain trust boundary.
...
## Bun 1.4 Capabilities

Consult the [Bun 1.4 release notes](https://bun.sh/blog/bun-v1.4) for the version-specific surface. Prefer these APIs and workflows only when they reduce a real dependency or operational burden; they do not remove normal input validation, isolation, or deployment verification.

### Stable runtime and data APIs

- **`Bun.WebView`** can drive a headless browser for deliberately scoped rendering, testing, or automation services. Treat browser navigation, downloaded data, page scripts, and target URLs as untrusted inputs; constrain navigation and egress, do not expose a general-purpose browser endpoint to callers, and use a dedicated service boundary for production automation.
- **`Bun.Image`** offers built-in image decode/transform/encode without a native addon. Validate upload type, dimensions, and size before processing; bound concurrency and memory use for user-provided images; store output outside executable/source paths.
- **`Bun.markdown`** provides Markdown rendering, but its HTML output is not a sanitizer. Sanitize rendered output before serving untrusted Markdown in a browser, and validate links/URI schemes separately.
- **`Bun.JSON5`**, **`Bun.JSONC`**, and **`Bun.JSONL`** remove common parser dependencies. Use JSON5/JSONC only for human-authored configuration that the project explicitly supports; keep machine-to-machine contracts strict JSON. For large JSONL ingestion, stream/chunk input, enforce record and total-size limits, and report malformed records without logging sensitive payloads.
- **`Bun.Terminal`** provides PTY-backed command automation. It is a process-execution boundary: use fixed command allowlists and argument arrays, never interpolate user input into a shell command, and avoid exposing terminal output that can contain secrets.
- **`Bun.cron()`** supports in-process and OS-backed scheduled work. In-process jobs end with the Bun process, so use a durable scheduler/worker where execution must survive restarts. Give schedules an explicit timezone, make jobs idempotent, bound concurrent work, and ensure every job has observable failures and a shutdown/disposal path.

### Parallel development and CI

- Use `bun run --parallel <script...>` only for independent scripts with no shared mutable output; prefer `--sequential` when ordering matters. In workspaces, scope parallel commands with explicit filters rather than broad globs.
- `bun test --parallel` runs test files in isolated worker processes. It can expose hidden state, database, port, and filesystem collisions: give each worker isolated resources and retain a non-parallel reproduction path. Use `--shard`, `--timings`, and `--changed` only after confirming their behavior against the repository's CI and test-selection policy.
- Keep parallel test/build performance claims evidence-backed. Record concurrency, workload, machine/runner class, and wall-clock results before changing CI defaults.

### Installation and dependency maintenance

The isolated linker (`bun install --linker=isolated`, or `[install] linker = "isolated"` in `bunfig.toml`) uses a global virtual store and changes the dependency layout. It is opt-in: do not enable it silently in an existing repository. Test workspace tooling, containers, deployment packaging, and any code that incorrectly assumes a particular `node_modules` shape before adopting it.

Use package-maintenance commands such as `bun audit fix`, `bun dedupe`, and `bun prune` as reviewed dependency changes, not blind cleanup. Inspect the resulting manifest, lockfile, and application tests; do not accept automated upgrades or removals without understanding their production impact.

### Experimental and opt-in transport APIs

Do not recommend HTTP/3 in `Bun.serve`, HTTP/2/HTTP/3 `fetch()` options, or `node:quic` as production defaults solely because they are available in Bun 1.4. These features have explicit flags and compatibility limitations. Gate them behind an intentional experiment, verify client/proxy/load-balancer behavior, define rollback, and keep a stable HTTP/1.1 or HTTP/2 path until production evidence supports promotion.
...
- Use `bun test --parallel` only after tests prove they do not share ports, databases, filesystem state, environment mutation, or module-level singletons. Isolate per-worker resources and retain a serial failure-reproduction command.
- For scaled CI, evaluate `--timings`, `--shard`, and `--changed` against the repository's selection policy; do not let a changed-test shortcut replace required integration or regression coverage.
...
- `Bun.Image`, `Bun.markdown`, JSON5/JSONC/JSONL, `Bun.Terminal`, and `Bun.cron()` when their trust boundaries and lifecycle constraints fit the service
...
- Sanitize `Bun.markdown` output before browser rendering; it does not sanitize untrusted HTML or unsafe URLs.
- Treat WebView and Terminal automation as privileged execution surfaces, not ordinary request handlers.
- Make `Bun.cron()` jobs observable, idempotent, timezone-explicit, and safe across process restarts.
...
Bun 1.4 materially improves Node.js compatibility, including common HTTP, filesystem, stream, test, observability, and ecosystem paths. That raises the value of a low-risk compatibility trial, but it is not proof for a specific native dependency, framework adapter, deployment target, or test suite.
...
11. **Rendering untrusted Markdown without sanitization.** `Bun.markdown` renders; it does not make browser output safe.
12. **Exposing WebView or Terminal as a generic user-controlled service.** Both can turn request data into browser or process authority; constrain their inputs and deployment boundary.
13. **Treating in-process cron as durable infrastructure.** It stops with the process; use appropriate supervision or a durable scheduler for critical work.
14. **Enabling the isolated linker or parallel CI as a no-risk speed tweak.** Both change behavior that can expose tooling assumptions and resource collisions.
15. **Shipping experimental HTTP/3 or QUIC by default.** Keep an explicit experiment and rollback path until the deployed topology is proven.
...
- [ ] Bun 1.4-specific APIs are chosen intentionally and their input, browser/process, rendering, scheduler, or storage boundaries are reviewed
- [ ] Markdown is sanitized before untrusted content is browser-rendered; WebView and Terminal automation are narrowly scoped
- [ ] Parallel scripts/tests and isolated-linker behavior have been verified in the project's CI, workspace, container, and deployment context before adoption
- [ ] Experimental transport features remain explicitly gated and have a rollback path
