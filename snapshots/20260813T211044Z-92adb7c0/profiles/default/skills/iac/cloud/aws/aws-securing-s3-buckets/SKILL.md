---
name: aws-securing-s3-buckets
description: "Use when the task involves aws securing s3 buckets in AWS. >"
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
  hermes:
    tags: [aws, iac, s3, security, storage]
    related_skills: [cloud-architect, terraform-infrastructure, aws-troubleshooting-s3-files]
---

# Aws Securing S3 Buckets

## Overview

This skill covers aws securing s3 buckets workflows in AWS and was adapted from `aws/agent-toolkit-for-aws/specialized-skills/storage-skills/securing-s3-buckets` into Hermes-native skill format for the local `iac/cloud/aws` taxonomy.

## When to Use

- Use this skill when the task matches the AWS scope described by `aws-securing-s3-buckets`.
