---
name: macos-cleaner
description: Use when analyzing or reclaiming disk space on a macOS Hermes host. Emphasizes safe diagnosis, impact-aware recommendations, and explicit user confirmation before any destructive cleanup.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
  hermes:
    tags: [macos, disk-cleanup, storage, mole, tmux, safety]
...
- The user says their macOS Hermes host is low on disk space.
- The user wants to understand what is consuming storage before deleting anything.
- The user wants a cautious cleanup plan that distinguishes safe deletions from useful caches.
- The task involves Mole-based analysis, tmux-driven TTY workflows, or macOS-specific cleanup guidance.

Do not use this skill for Windows or Linux cleanup, for silent/automatic deletion, or when the user specifically wants aggressive unattended pruning.
...
**IMPORTANT**: Mole requires TTY. Always use tmux from Hermes or another terminal automation environment.
...
Use direct shell commands for these checks rather than relying on bundled helper scripts, unless you add and verify local support scripts in this skill directory first.
...
```bash
# Top cache directories in the user library
find ~/Library/Caches -mindepth 1 -maxdepth 1 -type d -print0 | xargs -0 du -sh 2>/dev/null | sort -hr | head -20

# User logs
find ~/Library/Logs -mindepth 1 -maxdepth 1 -print0 | xargs -0 du -sh 2>/dev/null | sort -hr | head -20
```
...
4. Spot-check candidate folders with `du -sh` and recent modification times before recommending deletion
...
```bash
find ~ -type f -size +100M -not -path '*/.Trash/*' -print0 2>/dev/null | xargs -0 du -h 2>/dev/null | sort -hr | head -50
```
...
1. **TTY Environment**: Mole requires a TTY for interactive commands. Use `tmux` when running from Hermes, terminal automation, or scripts.
...
**Using Mole with tmux (REQUIRED for Hermes terminal automation):**
...
```python
# Example confirmation helper pattern
...
# Calculate space recovered by comparing pre/post `df -h /` output
```
...
This skill currently ships only with reference documents. If you later add reusable helper scripts, place them under `scripts/` inside this skill directory and update the examples in `SKILL.md` to match the files that actually exist.
...
Workflow:
1. Inspect Homebrew, npm, Docker, and other developer storage with the direct commands documented above
...
Workflow:
1. Find large files with the direct inspection command documented above
...
1. **Trusting category names without verifying the canonical taxonomy.** A manually added skill can sit under a near-miss directory like `local-util` even when the intended category is `local-utils`. Normalize the directory first, then verify discovery in Hermes before reviewing content.
2. **Leaving imported environment wording in place.** If the skill still says another agent runtime such as Claude Code, patch it to Hermes-native or generic terminal wording in both `SKILL.md` and support files.
3. **Referencing helper scripts that do not exist locally.** If examples mention `scripts/...` helpers but the skill directory only contains `references/`, replace those examples with direct commands or add the actual scripts.
4. **Stopping after the main file.** After editing a manually added skill, run a stale-reference sweep across the whole skill directory so support docs do not keep old tool names, categories, or nonexistent file references.

## Verification Checklist

- [ ] Category directory matches the intended taxonomy (for example `local-utils`, not a near-miss variant)
- [ ] `skills_list(category=...)` shows the skill under the normalized category
- [ ] Frontmatter uses Hermes peer metadata (`name`, `description`, `version`, `author`, `license`, `metadata.hermes`)
- [ ] Main file and support docs use Hermes-native or generic terminal wording
- [ ] Examples reference files and commands that actually exist
- [ ] A stale-reference search across the full skill directory comes back clean for imported residue and broken local pointers
...
- **APFS snapshots aren't deletable by `rm`** — use `tmutil deletelocalsnapshots`. A cleaner reporting freed bytes via `df` may see no change because snapshots still hold the blocks.
- **iCloud "Optimize Mac Storage" silently re-downloads cleaned files** — whitelist excluded paths or the cleaner runs in a loop.
- **TimeMachine local snapshots compete with iCloud for the same blocks** — conflict resolution is non-deterministic; "freed" space may reappear within an hour.
- **`/private/var/db/` caches mustn't be cleaned without understanding** — some are rebuilt on reboot (cheap), some require reinstall (expensive).
- **`du` vs `df` on APFS** show very different numbers due to clones — `du` reports apparent size; `df` reports blocks used. Don't trust either alone.
- **Quarantined files (`com.apple.quarantine` xattr)**: removing without considering Gatekeeper may re-add the attribute; some workflows loop.
