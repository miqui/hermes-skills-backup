...
This skill covers creating amazon aurora db cluster with instances workflows in AWS and was adapted from `aws/agent-toolkit-for-aws/specialized-skills/database-skills/creating-amazon-aurora-db-cluster-with-instances` into Hermes-native skill format for the local `iac` taxonomy.
