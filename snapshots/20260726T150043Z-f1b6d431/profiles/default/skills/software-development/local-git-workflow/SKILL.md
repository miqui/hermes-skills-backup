...
2. Capture the current all-profile corpus with `hsb-snapshot`; run its normal secret scan first. If strict capture aborts, inspect only the reported path/category/line evidence—never print matched values. Use the repository's documented `--no-secrets-check` exception only after confirming the findings are previously reviewed documentation/example placeholders (for example, token-shaped teaching values) rather than runtime secret stores or real credentials.
...
6. Verify the remote branch SHA, PR URL, clean worktree, and the separate Actions jobs for tests, changed-snapshot detection, and changed-snapshot validation.
