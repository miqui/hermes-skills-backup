...
This skill covers aws transform (atx) workflows in AWS and was adapted from `aws/agent-toolkit-for-aws/specialized-skills/migration-and-modernization-skills/aws-transform` into Hermes-native skill format for the local `iac/cloud/aws` taxonomy.

## When to Use

- Use this skill when the task matches the AWS scope described by `aws-transform`.
