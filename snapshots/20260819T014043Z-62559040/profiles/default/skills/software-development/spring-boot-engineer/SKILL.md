...
When the task involves Kafka producers/consumers, Kafka Streams, dead-letter topics, or exactly-once transactional processing, load `references/kafka.md`.

When the task involves batch jobs, chunk-oriented steps, readers/writers, partitioning, or batch scheduling, load `references/batch.md`.

When the task involves LLM chat models, embeddings, vector stores, RAG pipelines, or function calling with Spring AI, load `references/ai.md`.
...
17. **Auto-commit in Kafka consumers.**
    If `enable-auto-commit: true` (default) and processing fails after commit, the message is lost. Use manual ack for at-least-once processing.

18. **No DLQ strategy in Kafka consumers.**
    A poison message that always fails blocks the partition indefinitely. Always configure a `DefaultErrorHandler` with `DeadLetterPublishingRecoverer`.

19. **Spring Batch auto-running on startup.**
    `spring.batch.job.enabled` defaults to `true`, running all jobs at boot. Set to `false` and launch explicitly via scheduler or API.

20. **No similarity threshold in RAG.**
    Without `withSimilarityThreshold()`, the vector store returns low-relevance results that pollute context. Set a threshold (0.7 is reasonable).

21. **Embedding model mismatch after model switch.**
    If you switch embedding models, existing vectors are incompatible. Re-embed all documents.
...
- [ ] If deeper topic guidance is needed, the right reference file is loaded (`web.md`, `data.md`, `advanced-data.md`, `security.md`, `security-advanced.md`, `cloud.md`, `observability.md`, `native-image.md`, `modulith.md`, `kafka.md`, `batch.md`, `ai.md`, `testing.md`)
