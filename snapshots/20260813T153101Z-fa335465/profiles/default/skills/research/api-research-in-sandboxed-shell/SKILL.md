...
## Verifying a local clone is in sync with its GitHub remote (without downloading/diffing content)

When the task is "is this local file the canonical/current one on GitHub" — e.g. confirming a locally-generated data artifact matches what's published — you don't need to `git pull`, fetch the blob, or diff bytes. The Contents API returns `size` and a git blob `sha` cheaply:

```bash
gh api "repos/<owner>/<repo>/contents/<path>?ref=<branch>" \
  --jq '{sha, size, html_url, download_url}'
```

Compare `size` (and, if you need certainty beyond a byte-count coincidence, the blob `sha`, which is `git hash-object` over the file — not a plain sha256) against the local file (`ls -la` / `wc -c`). Matching size is strong-but-not-certain sync evidence; for real certainty compute `git hash-object <local-file>` and compare it to the API's `sha` directly — this confirms local and remote are byte-identical without ever transferring the file content itself.

Also use `git log --oneline -- <path>` and `git status --short` locally first — a clean `git status` plus the last commit on that path already tells you there's nothing uncommitted, and the remote check above then confirms nothing has diverged upstream either. Together these two calls answer "is local === remote" without a full clone/pull.
