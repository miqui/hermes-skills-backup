---
name: aws-storing-and-querying-vectors
description: "Use when the task involves store and query vectors with amazon s3 vectors in AWS. >-"
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
  hermes:
    tags: [aws, iac, vectors, search, analytics]
    related_skills: [cloud-architect, aws-querying-data-lake, aws-sdk-python-usage]
---
...
This skill covers store and query vectors with amazon s3 vectors workflows in AWS and was adapted from `aws/agent-toolkit-for-aws/specialized-skills/storage-skills/storing-and-querying-vectors` into Hermes-native skill format for the local `iac` taxonomy.

## When to Use

- Use this skill when the task matches the AWS scope described by `aws-storing-and-querying-vectors`.
