...
metadata:
  hermes:
    ...
    related_skills: [systematic-debugging, plan, subagent-driven-development, code-performance-engineering]
---
...
### With code-performance-engineering

Performance changes still need correctness tests. Benchmarks and profiles prove speed or resource improvements, but they do not replace behavior tests. When optimizing a hot path, pair this skill with `code-performance-engineering`: write or preserve tests for correctness first, then add benchmark or regression guardrails where the project supports stable performance checks.
