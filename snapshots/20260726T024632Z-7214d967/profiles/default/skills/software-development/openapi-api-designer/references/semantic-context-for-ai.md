...
Use only the keys defined in `assets/extension-vocabulary.md`. New `x-` keys go in that file first, with a description, before they appear in a spec. This prevents vocabulary drift, which is what makes AI consumption hard across multiple specs.
