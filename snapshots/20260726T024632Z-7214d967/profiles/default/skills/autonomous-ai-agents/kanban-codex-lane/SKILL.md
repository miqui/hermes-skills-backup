...
- [ ] Hermes reviewed `git -C <WORKTREE> diff --stat` first, then selected-file diffs with `git -C <WORKTREE> diff -- <path>`.
...
- [ ] Hermes reviewed diff summaries first, then selected diffs and safety-sensitive files.
