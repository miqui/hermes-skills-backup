/* Minimal plain Node.js coding-agent starter.
 *
 * Uses the OpenAI-compatible client style in JavaScript.
 * This version is intentionally plain Node, without TypeScript.
 *
...
function runTestsTool(scope = "") {
  try {
    const args = scope ? [scope] : [];
    const stdout = execFileSync("pytest", args, {
      cwd: REPO_ROOT,
      encoding: "utf8",
...
function applyPatchTool(filePath, newContent) {
  try {
    const full = safePath(filePath);
