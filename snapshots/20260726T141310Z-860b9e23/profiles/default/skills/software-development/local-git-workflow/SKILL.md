...
2. Capture the current all-profile corpus with `hsb-snapshot`; run its normal secret scan first. Only use the repository's documented `--no-secrets-check` exception when the scan is blocked by previously reviewed documentation/example placeholders.
3. Validate the new snapshot with both `hsb-verify --snapshot-id <id>` and `hsb-validate --snapshot-id <id>`, then run the backup project's regression suite.
