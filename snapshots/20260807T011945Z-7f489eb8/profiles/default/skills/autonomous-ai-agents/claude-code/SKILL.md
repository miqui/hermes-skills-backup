...
### Subscription-backed worker calls on an API-key host

If the intended route is an authenticated Claude Code subscription but the host also exports `ANTHROPIC_API_KEY` or `ANTHROPIC_TOKEN`, remove those variables for the `claude` subprocess. Otherwise Claude Code may choose direct API billing instead of its stored OAuth login:

```bash
env -u ANTHROPIC_API_KEY -u ANTHROPIC_TOKEN \
  claude -p 'Review the selected files and return findings only.' \
  --allowedTools 'Read' --max-turns 5
```

Verify the desired OAuth login beforehand with `claude auth status --text`. Do **not** use `--bare` for subscription-backed calls: bare mode bypasses OAuth and requires an API key. Keep API-key and subscription-backed workers explicit and separate.

Print mode skips all interactive dialogs—no workspace trust prompt and no permission confirmations. This is the cleanest integration path.
...
# Scope a Git change before piping patches into Claude
terminal(command="git diff HEAD~3 --stat && git diff HEAD~3 --name-only", timeout=60)
# After selecting a bounded file scope:
terminal(command="git diff HEAD~3 -- src/auth.py | claude -p 'Summarize these changes' --max-turns 1", timeout=60)
```
...
### Hermes Controller Repair Loop

When Hermes is orchestrating a bounded coding task through Claude Code print mode, do not treat `Reached max turns` as a failed pattern by default. Often Claude has already created a mostly-correct partial implementation.

Recommended controller sequence:
1. inspect repo state after the first pass (`git status --porcelain=v1`, key files changed)
2. verify outside Claude with a real targeted test, then the broader suite
3. if verification fails, send Claude the **exact failing command and traceback/output** and ask for the smallest fix needed
4. re-run verification independently before reporting success

This produces a stronger orchestration story than a blind retry loop: Hermes scopes, Claude codes, Hermes verifies, Claude repairs, Hermes confirms.

Important shell-safety note: if your repair prompt includes markdown backticks, inline code, or tracebacks, do not paste it raw into a double-quoted shell string. The shell may interpret backticks before `claude -p` runs. Prefer a heredoc/file-fed prompt or plain-text failure transcript with backticks removed.

For a reusable write-up and safer prompt shape, see `references/hermes-controller-repair-loop.md`.
...
## Node/TypeScript App Embedding

When Claude Code is being used *inside* a Node/TypeScript application rather than as a human-operated coding session, also load `references/node-typescript-cli-integration.md`.

Key rule: do **not** assume `@anthropic-ai/claude-code` always exposes a stable importable JavaScript SDK surface in the target environment. Verify the installed package exports first. If the package behaves as a CLI wrapper, call `claude -p --output-format json <prompt>` through a small typed subprocess adapter, parse the JSON `result` field, and provide a deterministic fallback when Claude is unavailable.
...
```
# Inspect compact scope first.
terminal(command="cd /path/to/repo && git diff main...feature-branch --stat && git diff main...feature-branch --name-only", timeout=60)
# Then send a bounded, selected file scope to Claude.
terminal(command="cd /path/to/repo && git diff main...feature-branch -- src/auth/login.py | claude -p 'Review this diff for bugs, security issues, and style problems. Be thorough.' --max-turns 1", timeout=60)
```
...
| `SessionStart` | When a session begins | Load dev context (e.g., `git status --porcelain=v1`) |
...
13. **`@anthropic-ai/claude-code` may behave like a CLI wrapper in app projects** — when embedding Claude into a Node/TypeScript app, verify the package's actual exports before coding against a direct SDK import. If needed, shell out to `claude -p --output-format json` and parse the final `result` field instead.
