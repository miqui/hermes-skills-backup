...
metadata:
  hermes:
    ...
    related_skills: [api-governance, node-backend, code-performance-engineering, test-driven-development, requesting-code-review, application-authorization, apollo-federation-graphos]
---
...
### Federated graph delivery: make the composed graph the contract

Individual subgraph unit tests are necessary but insufficient: the Router/composed graph is the client-facing contract.

Before publishing or merging a federation change:

1. **Inventory topology.** Record every subgraph, its owning domain, port/URL, entity keys, and whether it contributes, extends, or resolves each shared entity field.
2. **Compose before release.** Run the platform's schema-composition or compatibility check in CI before any publish step. Treat composition errors and incompatible changes as merge blockers.
3. **Validate named operations against the composed endpoint.** Exercise representative cross-subgraph queries and mutations through the Router, asserting transport behavior and GraphQL `data`/`errors`; do not validate only direct subgraph endpoints.
4. **Keep artifacts synchronized.** Validate checked-in operation documents, frontend query strings, README examples, and API samples against the current composed schema. A schema rename that leaves an example using an old field, input, or payload name is a contract defect.
5. **Use least-privilege publication credentials.** Keep graph-management keys in CI secret/context storage; never commit, log, or pass them in client-visible configuration. Verify graph reference, variant, subgraph name, schema path, and routing URL before publishing.
6. **Verify local multi-service readiness.** Confirm each subgraph is healthy, the Router targets intended endpoints, CORS permits only needed local/test origins, and the frontend calls the Router rather than a subgraph directly.

For Apollo Federation 2, Apollo Server v4 subgraphs, Apollo Router, Rover, or GraphOS, pair this framework-neutral protocol with `apollo-federation-graphos` for the vendor-specific runbook. It intentionally complements this skill rather than replacing it.
