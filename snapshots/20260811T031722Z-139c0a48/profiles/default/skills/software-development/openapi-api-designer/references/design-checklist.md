...
- [ ] File saved to the agreed output path (project-local path preferred when one exists).
- [ ] `python scripts/validate.py <file>` passes when those local scripts are part of the active workflow.
- [ ] `python scripts/lint.py <file>` passes when those local scripts are part of the active workflow, or any failures are surfaced to the user with rationale.
- [ ] Response message names assumptions and points the user at the final file path or attached artifact.
