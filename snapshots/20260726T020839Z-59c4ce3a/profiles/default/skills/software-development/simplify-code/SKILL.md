...
Pick the diff source by what the user asked for, then inspect its compact scope
before capturing any patch text. The default source order is:
...
git diff HEAD~1 --stat             # "the last commit"
git diff main...HEAD --stat        # "this branch" / "my PR"
git diff -- src/foo.py             # specific file(s)
```

For any selected source, run the matching `--name-only` and `--shortstat`
variant before reading a patch.

If the selected summary is empty and there's no git repo or no changes, fall
back to the files the user explicitly named or that were recently created or
edited in this session. If you genuinely can't find any changed code, say so
and stop — there's nothing to simplify.

Read `--shortstat` before capturing a full patch. If the selected diff exceeds
2,000 changed lines, stop and ask the user to narrow it by directory,
file, or commit before proceeding. Do not capture the full diff or launch
reviewers until the scope is narrowed. For a bounded scope, capture the full
selected diff text only after this check.
