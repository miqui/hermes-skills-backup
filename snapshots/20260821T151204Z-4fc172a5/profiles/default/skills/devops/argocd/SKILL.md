...
metadata:
  hermes:
    tags: [argocd, gitops, kubernetes, deployment, applicationset, cli, api]
    related_skills: [argocd-advanced, argo-workflows, deployment-engineer]
---
...
Use this skill for the core operational surface of Argo CD: authenticating, managing Applications, Projects, repositories, clusters, accounts, and routine sync or health troubleshooting.

This skill is the primary routing point for normal Argo CD work. For deeper ApplicationSet automation, image updater flows, and cluster bootstrapping, route to `argocd-advanced`. For workflow execution engines and pipeline DAGs on Kubernetes, route to `argo-workflows`.

## When to Use

- Creating, syncing, diffing, waiting on, rolling back, or deleting Argo CD Applications
- Managing Argo CD Projects, repositories, clusters, and account tokens
- Using the Argo CD CLI or REST API for day-to-day GitOps operations
- Troubleshooting app health, sync status, repo connectivity, cluster auth, and RBAC issues

Do not use this skill for:
- Argo Workflows DAG/steps/CronWorkflow authoring or Hera Python SDK work
- Advanced ApplicationSet platform patterns, image updater automation, or new-cluster bootstrapping when `argocd-advanced` is the better router
...
| Subject | Path |
|---------|------|
| Hypera/Cafehyna local cluster context (kubeconfig, port-forward, aliases) | `References/command-workflows/` + `References/command-tools/` |
...
For multi-cluster ApplicationSets, Image Updater, cluster bootstrapping, and workload onboarding, see the sibling **`argocd-advanced`** skill. For workflow orchestration, DAGs, steps, CronWorkflows, artifact passing, and Hera Python authoring, see **`argo-workflows`**. For general deployment strategy and rollout design, see **`deployment-engineer`** or **`deployment-pipeline-design`**.
