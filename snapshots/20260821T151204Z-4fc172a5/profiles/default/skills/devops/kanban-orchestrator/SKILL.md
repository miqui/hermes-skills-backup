...
metadata:
  hermes:
    tags: [kanban, multi-agent, orchestration, routing, delegation, workflow, task-assignment, dispatcher]
...
**A direct stop/done signal is terminal.** When the user says “stop,” “we are done,” or equivalent, stop active work immediately: do not launch another review, audit, packaging action, status check, or explanatory follow-up. Give one short acknowledgement and remain silent unless the user starts a new request. If a repository has a mandatory append-only activity log, write at most the single required closure entry before acknowledging; never describe that bookkeeping as active analysis or use it to prolong the interaction.
